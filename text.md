...
Для работы с содержанием запроса NestJS предоставляет вот такой набор декораторов, которые позволяют нам получить, например, всю строку запроса, или выдернуть какой-то элемент этой строки, передав его название в декоратор `@Query("access_key")`
